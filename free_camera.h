...
public:
	...
	inline virtual void DisplayCameraPosition() const;
	...
	inline virtual void AddYaw(const float dt, const float value) { yaw_ += value * dt; Update(); }
	inline virtual void subtractYaw(const float dt, const float value) { yaw_ -= value * dt; Update(); }

	inline virtual float GetPitch() const { return pitch_; }
	inline virtual void SetPitch(const float arg) { pitch_ = arg; Update(); }
	inline virtual void AddPitch(const float dt, const float value) { pitch_ += value * dt; Update(); }
	inline virtual void subtractPitch(const float dt, const float value) { pitch_ -= value * dt; Update(); }

	inline virtual float GetRoll() const { return roll_; }
	inline virtual void SetRoll(const float arg) { roll_ = arg; Update(); }
	inline virtual void AddRoll(const float dt, const float value) { roll_ += value * dt; Update(); }
	inline virtual void subtractRoll(const float dt, const float value) { roll_ -= value * dt; Update(); }
